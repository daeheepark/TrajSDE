...
class LocalEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.temporal_encoder = TemporalEncoder(historical_steps=self.historical_steps,
                                                embed_dim=self.embed_dim,
                                                num_heads=self.num_heads,
                                                dropout=self.dropout,
    ...
    def forward(self, data: TemporalData) -> torch.Tensor:
        ...
        # is_intersections, turn_directions, traffic_controls = data['is_intersections'][:,0,0], data['turn_directions'][:,0,0], data['traffic_controls'][:,0,0]
        ...
        #                       traffic_controls=traffic_controls, rotate_mat=data['rotate_mat'])
        out = self.al_encoder(x=(lane_feat, out), edge_index=edge_index, edge_attr=edge_attr,
...
class AAEncoder(MessagePassing):
    ...
    def __init__(self,
        ...
        self.input_diff = kwargs['input_diff']
        ...
        if self.parallel:
            ...
            if self.input_diff:
                center_embed = torch.where(bos_mask.t().unsqueeze(-1),
                                        ...
                                        center_embed)
            center_embed = center_embed.contiguous().view(x.shape[0], -1)
            ...
            if self.input_diff: center_embed = torch.where(bos_mask.unsqueeze(-1), self.bos_token[t], center_embed)
