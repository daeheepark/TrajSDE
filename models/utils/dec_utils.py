...
def cluster_traj(k: int, traj: torch.Tensor, interval: int = 3):
    """
    ...
    lbls = torch.as_tensor(np.array(cluster_lbls), device=traj.device).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, traj_len, 2).long()
    traj_summed = torch.zeros(batch_size, k, traj_len, 2, device=traj.device).scatter_add(1, lbls, traj)
    cnt_tensor = torch.as_tensor(np.array(cluster_counts), device=traj.device).unsqueeze(-1).unsqueeze(-1).repeat(1, 1, traj_len, 2)
    traj_clustered = traj_summed / cnt_tensor
    scores = 1 / torch.as_tensor(np.array(cluster_ranks), device=traj.device)
