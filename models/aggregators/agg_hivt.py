...
class GlobalInteractor(nn.Module):
    ...
    def __init__(self,
        ...
        if self.rotate:
            self.rel_embed = MultipleInputEmbedding(in_channels=[self.edge_dim, self.edge_dim], out_channel=self.embed_dim)
        else:
            self.rel_embed = SingleInputEmbedding(in_channel=self.edge_dim, out_channel=self.embed_dim)
        self.global_interactor_layers = nn.ModuleList(
            [GlobalInteractorLayer(embed_dim=self.embed_dim, num_heads=self.num_heads, dropout=self.dropout)
             for _ in range(self.num_layers)])
        self.norm = nn.LayerNorm(self.embed_dim)
        self.multihead_proj = nn.Linear(self.embed_dim, self.num_modes * self.embed_dim)
